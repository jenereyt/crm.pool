...
export async function loadClasses() {
  ...
  classList.addEventListener('click', async (e) => {
    ...
    } else if (e.target.classList.contains('class-attendance-btn')) {
      const classId = e.target.getAttribute('data-id');
      const cls = scheduleData.find(cls => cls.id === classId);
      const clientsList = await getClients();
    ...
    if (e.target.classList.contains('attendance-select')) {
      ...
      if (cls) {
        ...
        if (status === 'Пришёл') {
          const clientData = clients.find(c => c.name === client);
          if (clientData) {
            const sub = (await getActiveSubscriptions()).find(s => s.clientId === clientData.id && s.remainingClasses !== Infinity);
    ...
    document.getElementById('attendance-save-btn').addEventListener('click', () => {
      ...
      cls.clients.forEach(client => {
        ...
        if (attendance[client] === 'Пришёл') {
          const clientData = clients.find(c => c.name === client);
          if (clientData) {
            const sub = (getActiveSubscriptions()).find(s => s.clientId === clientData.id && s.remainingClasses !== Infinity);
            if (sub && sub.remainingClasses > 0) {
              sub.remainingClasses -= 1;
            }
          }
